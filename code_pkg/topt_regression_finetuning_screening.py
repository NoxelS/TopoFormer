...
class DataTrainingArguments:
    ...
    validation_data: Optional[str] = field(default=None, metadata={"help": "A folder containing the validation data."})
    validation_label: Optional[str] = field(default=None, metadata={"help": "A folder containing the validation data."})
...
def TopT_Finetuning():
    ...
    if data_args.train_data is not None:
        ...
        # positive
        positive_idx = np.where(train_label >= 0)[0]
        train_label = train_label[positive_idx]
        train_file_to_data = train_file_to_data[positive_idx, :, :, :]

        # additional train set
        v2015_label = pd.read_csv(
            r'/home/chendo11/workfolder/TopTransformer/TopFeatures_Data/downstream_task_labels/v2015_refine_train_label.csv',
            header=0, index_col=0,
        )
        train_label = np.append(train_label, v2015_label.values)
        
        v2015_data_dict = np.load(
            r'/home/chendo11/workfolder/TopTransformer/TopFeatures_Data/pretrain_feature_filtration212/v2015_train_feat.npy',
            allow_pickle=True,
        ).item()
        v2015_data = np.array([v2015_data_dict[k] for k in v2015_label.index])[:, :, 0::2, :]
        train_file_to_data = np.vstack([train_file_to_data, v2015_data])
