...
def TopT_Finetuning():
    ...
    if data_args.train_data is not None:
        ...
        train_file_to_data_dict = np.load(data_args.train_data, allow_pickle=True).item()
        train_file_to_data = [train_file_to_data_dict[k] for k in train_list]
    ...
    if data_args.validation_data is not None:
        ...
        valid_file_to_data_dict = np.load(data_args.validation_data, allow_pickle=True).item()
        valid_file_to_data = [valid_file_to_data_dict[k] for k in valid_list]
        print('here'*100)
        print(data_args.validation_data)
        print(data_args.validation_label)
        print(np.shape(valid_label))
        print(np.shape(valid_file_to_data))
